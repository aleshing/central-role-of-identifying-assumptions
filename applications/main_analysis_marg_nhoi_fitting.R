...
pi_0_dir <- get_pi_0_marg_hoi(pi_tilde_dir, K, list_1 = 2, list_2 = 4, xi = 1)
set.seed(61)
...
N_dir <- rbind(N_dir, 
               data.frame(N = n + n_0_sampler_isp(pi_0_dir[pi_0_dir > 0], n),
                          prior = length(M) + 1))
...
pi_tilde_ll <- ll_fit$draws() %>% 
    ...
    merge_chains() %>% as_draws_df() %>% 
    select(-c(.chain, .iteration, .draw)) %>% as.matrix() %>% unname()
pi_0_ll <- get_pi_0_marg_hoi(pi_tilde_ll, K, list_1 = 2, list_2 = 4, xi = 1)
set.seed(61)
...
N_ll <- rbind(N_ll, 
              data.frame(N = n + n_0_sampler_isp(pi_0_ll[pi_0_ll > 0], n),
                         prior = length(M) + 1))
...
pi_0_lcmcr <- get_pi_0_marg_hoi(pi_tilde_lcmcr, K, list_1 = 2, list_2 = 4, 
                                xi = 1)
set.seed(61)
...
N_lcmcr <- rbind(N_lcmcr, 
                 data.frame(N = n + 
                                n_0_sampler_isp(pi_0_lcmcr[pi_0_lcmcr > 0], n),
                            prior = length(M) + 1))
...
pi_0_conting <- get_pi_0_marg_hoi(pi_tilde_conting, K, list_1 = 2, list_2 = 4, 
                                  xi = 1)
set.seed(61)
...
N_conting <- rbind(N_conting, 
                   data.frame(N = n + n_0_sampler_isp(pi_0_conting[pi_0_conting > 0], n),
                              prior = length(M) + 1))

#### Save the relevant output ####
save(pi_tilde_dir, pi_tilde_ll, pi_tilde_lcmcr, pi_tilde_conting, 
     file = "marg_nhoi_fits/pi_tilde_samples.RData")
save(N_dir, N_ll, N_lcmcr, N_conting, file = "marg_nhoi_fits/N_samples.RData")
