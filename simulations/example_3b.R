...
# Example 3b using lists 2-5 from simulation of Manrique-Vallier (2016)
        ...
        results_pi_0[[i]][j, ] <- c(mean(pi_0_dpmm), 
                                    quantile(pi_0_dpmm, 
                                             prob = c(0.5, 0.025, 0.975,
                                                      0.25, 0.75)))
        if(j %% 10 == 0){
            save(results_N, results_pi_0, file = "example_3b_simulation.RData")
        }
    }
}
...
for(i in 1:length(Ns)){
    print(paste0("Example 3b: Population Size " , Ns[i], " Results"))
