...
# Example 4a using lists 2-4 from simulation of Manrique-Vallier (2016) and
...
q <- matrix(c(0.033, 0.099, 0.132, 
              0.25, 0.2, 0.3, 
              0.825, 0.759, 0.99), ncol = K, byrow = TRUE)
# Cell probabilities
        ...
        results_pi_0[[i]][j, ] <- c(mean(pi_0_dpmm), 
                                    quantile(pi_0_dpmm, 
                                             prob = c(0.5, 0.025, 0.975,
                                                      0.25, 0.75)))
        if(j %% 10 == 0){
            save(results_N, results_pi_0, file = "example_4a_simulation.RData")
        }
    }
}
...
for(i in 1:length(Ns)){
    print(paste0("Example 4a: Population Size " , Ns[i], " Results"))
